...
class MakeRunList extends Command
{
    ...
    public function handle()
    {
        ...
        if ($dataOrder == true || $dataOrderFront == true) {
            ...
            if($this->insertBucket($dateNow)){
                $this->hitungJumlahPrice($dateNow);
            }
        }
    ...
    {
        /**
         * setiap insert run list ada perhitungan price
         * jika jumlah price saat ini sudah lebih dari 8000
         * maka akan di schedulenya besoknya sekalin jumat, sabtu minggu
         */
        //cek tanggal order, cari yg belum lewat hari ini
        $dataOrder = DB::table('orders as o')
            ->select('coi.order_item_id', 'coi.component_id', 'coi.color_id', 'coi.id as component_order_item_id')
                ...
                for ($i = 0; $i < $jml_data; $i++) {
                    ...
                    if ($schedule == $schedule_old && $i != 0) { //update
                        ...
                        if ($run_list_id != $run_list_id_old && $i != 0) {
                            $run_list_u = $getBucket->run_list . ', Run ' . $week_u . '.' . $number_u;
    ...
    public function hitungJumlahPrice($dateNow)
    {
        $getDataBucket = Bucket::select('o.id', 'rl.bucket_id')
            ->join('run_lists as rl', 'rl.bucket_id', '=', 'buckets.id')
            ->join('run_list_details as rld', 'rld.run_list_id', '=', 'rl.id')
            ->join('component_order_items as coi', 'coi.id', '=', 'rld.component_order_item')
            ->join('order_items as oi', 'coi.order_item_id', '=', 'oi.id')
            ->join('orders as o', 'oi.order_id', '=', 'o.id')
            ->where('o.shipping_due_date', '>', $dateNow)
            ->where('buckets.value', '=', 0)
            ->where('o.status', 20)
            ->orderBy('o.id', 'asc')
            ->get();
        $jml_data = count($getDataBucket);
        if(!empty($getDataBucket) && $jml_data > 0){
            $order_id ='';
            $order_id_old ='';
            for ($i=0; $i < $jml_data; $i++) { 
                $order_id = $getDataBucket[$i]->id;
                if ($i != 0) { //cek data kedua dan seterusnya
                    $order_id_old = $getDataBucket[$i - 1]->id;
                } else { //untuk data pertama saja
                    $order_id_old = $getDataBucket[$i]->id;
                }

                if ($order_id != $order_id_old || $i == 0) {
                    ...
                    ->get();
                    $total_jml = 0;
                    $bucket_id = $getDataBucket[$i]->bucket_id;
                    ...
                    if (!empty($getBucket->id)) {
                        Bucket::where('id', $getBucket->id)->update([
                            'value' => (int)$getBucket->value + (int)$total_jml
                        ]);
                    }
                }
            }
